...
if test (uname) = Darwin
    test -f ~/.config/fish/os/darwin.fish; and source ~/.config/fish/os/darwin.fish
else
    test -f ~/.config/fish/os/linux.fish; and source ~/.config/fish/os/linux.fish
end

# Miscellaneous
alias sudo='sudo ' # Allows sudo for all aliases
alias cls='clear && printf "\e[3J"'
alias fishfig='$EDITOR ~/.config/fish/config.fish' # fixed: was invalid syntax + bogus path

# Information (localip is OS-specific — see os/{darwin,linux}.fish)
...
# SSH — pubkey alias is defined per-OS in os/{darwin,linux}.fish
